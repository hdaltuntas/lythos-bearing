...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        prog="lythos-bearing",
        description="Bearing capacity of shallow foundations: the general bearing "
                    "capacity equation with the factors of Terzaghi, Meyerhof, Hansen, "
                    "Vesić, EN 1997-1 and Skempton, eccentric and inclined loads, "
                    "layered and two-layer ground, in-situ and rock methods, seismic "
                    "loading, and parametric / reliability studies.")
    parser.add_argument("--version", action="version", version=f"{APP_NAME} {__version__}")
    # Running the program with no subcommand means "web", so the top level
    # carries that subcommand's defaults: without them the bare `lythos-bearing`
    ...
    run.add_argument("project", help="path to a .bearing / .json project file")
    ...
    study.add_argument("project", help="path to a .bearing / .json project file")
    ...
    example.add_argument("-o", "--out", default="project.bearing")
    ...
    try:
        ...
        # (a foundation below the profile, a layer with no strength, an
...
def _example_study(values: dict) -> list:
    ...
    return [
        {"path": "loading.V", "label": "Actions · V", "mode": "dist", "dist": "lognormal",
         "mean": values["V"], "cov": 0.15, "min": 0, "max": 0, "n_points": 5},
        {"path": "soil_profile.1.phi", "label": "Stiff clay · phi", "mode": "dist",
         "dist": "normal", "mean": clay["phi"], "cov": 0.12, "min": 0, "max": 0,
         "n_points": 5},
    ]
