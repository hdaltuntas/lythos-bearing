"use strict";
/* Lythos Bearing — the browser interface.
 *
 ...
 * when it applies — "only for a rectangle", "only when the earthquake is on" —
 * and the form redraws itself whenever a value another field depends on
 * changes, so what is on the screen is the method that is actually running.
 */
...
function collectWatched() {
  S.watched = new Set();
  for (const part of ["project", "foundation", "options", "study"]) {
...
function setValue(key, value) {
  S.values[key] = value;
  if (S.watched.has(key)) {
    renderForms();
    if (key === "shape") refreshVariables();
  }
}
...
function renderView() {
  ...
  if (S.module === "inputs") {
    ...
    if (which === "summary") {
      ...
      if (S.analysis.table) {
        parts.push(el("h2", { text: T("methods_title") }));
        parts.push(dataTable(S.analysis.table.columns, S.analysis.table.rows));
      }
      parts.push(plotImage("analysis", "schematic"));
      parts.push(plotImage("analysis", "comparison"));
...
async function downloadReport() {
  ...
  try {
    ...
    await download("/api/report", { format }, `lythosbearing_report.${format}`);
...
async function exportStudy(format) {
  try {
    await download("/api/export-study", { format }, `lythosbearing_study.${format}`);
    status(T("saved"));
  } catch (error) {
    status(`${T("error")}: ${error.message}`, true);
  }
...
async function saveProject() {
  try {
    await download("/api/project", { values: S.values }, "project.bearing");
    status(T("saved"));
  } catch (error) {
    status(`${T("error")}: ${error.message}`, true);
  }
...
async function applyTheme(theme) {
  ...
  try {
    localStorage.setItem("lythosbearing-theme", theme);
...
function storedTheme() {
  try {
    return localStorage.getItem("lythosbearing-theme");
  } catch {
    return null;
  }
}
