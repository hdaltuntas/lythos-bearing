...
A bearing capacity analysis takes a few milliseconds and runs inline. A
parametric or reliability study of a few thousand samples does not: it runs in
a thread, reports progress through `state()`, and can be cancelled.
"""
...
from ..engine import BearingAnalysis, BearingError
...
class Session:
    ...
    def __init__(self, lang: str = "en", theme: str = "light"):
        ...
        self.theme = theme
        self.analysis: Optional[BearingAnalysis] = None
    ...
    def _explain(self, exc: Exception) -> str:
        ...
        if isinstance(exc, BearingError):
    ...
    # ================================================================== analysis
    def _method_table(self, analysis: BearingAnalysis) -> dict:
        """The method comparison the summary view shows under the cards."""
        L = self.t
        res = analysis.results
        columns = [L["head_method"], L["head_analysis"], L["head_qult"], L["head_qnet"],
                   L["head_qall"], L["head_FS"]]
        rows = []
        applied = res["applied"]["q_net"]
        for key, entry in list(res["methods"].items()) + list(res["others"].items()):
            net = entry.get("q_net_ult")
            allowable = entry.get("q_all")
            if allowable is None and net is not None and math.isfinite(net):
                allowable = net / analysis.FS + res["surcharge"]["effective"]
            fs = (net / applied if net is not None and math.isfinite(net) and applied > 0
                  else float("nan"))
            analysis_name = entry.get("analysis") or entry.get("governing") or ""
            rows.append({
                "cells": [summary.method_label(L, key),
                          L.get(f"analysis_label_short_{analysis_name}", ""),
                          _pressure(entry.get("q_ult")), _pressure(net),
                          _pressure(allowable),
                          f"{fs:.2f}" if math.isfinite(fs) else "—"],
                "states": ["", "", "", "", "",
                           "" if not math.isfinite(fs)
                           else ("ok" if fs >= analysis.FS else "bad")],
                "primary": key == res["primary"],
            })
        return {"columns": columns, "rows": rows}

    def analyse(self, values: dict) -> dict:
        """Runs the bearing capacity analysis on the interface's values."""
        try:
            analysis = BearingAnalysis(forms.to_config(values))
            analysis.run()
        ...
        return {
            ...
            "table": self._method_table(analysis),
            "q_ult": res["q_ult"],
            "FS": res["checks"]["bearing"]["actual"],
            "required_width": res["required_width"],
        }
    ...
    def plot(self, target: str, kind: str, output: str = "") -> bytes:
        ...
        if target == "study":
            ...
            return render.figure_to_png(render.study_figure(
                study, kind, output or (outputs[0] if outputs else "FS"), self.lang, theme))
    ...
    def start_study(self, values: dict) -> dict:
        ...
        try:
            ...
            BearingAnalysis(cfg).run(with_width=False)   # the base case must be analysable
    ...
    def _study_table(self, study: Study) -> dict:
        ...
        header = [self.t.get(f"out_{column}", by_path.get(column, column))
