"""
lythosbearing.web.server — the local HTTP server that serves the interface.
...
#: A mark for the browser tab: a footing on layered ground, and the wedge
#: that fails beneath it
_FAVICON = (
    b'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">'
    b'<rect width="32" height="32" rx="7" fill="#F5F4ED"/>'
    b'<path d="M3 14h26v6H3z" fill="#E3C98F"/>'
    b'<path d="M3 20h26v9H3z" fill="#A7B8A0"/>'
    b'<path d="M9 9h14v5H9z" fill="#8A8680"/>'
    b'<path d="M9 14 16 24 23 14z" fill="#C9A876" stroke="#A26A12" stroke-width="1.2" '
    b'stroke-linejoin="round"/>'
    b'<path d="M16 2v6" stroke="#C6613F" stroke-width="2.4" stroke-linecap="round"/>'
    b'<path d="M12.5 5.5 16 9l3.5-3.5" fill="none" stroke="#C6613F" stroke-width="2.2" '
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _project(self, data: dict) -> None:
        ...
        self._download("project.bearing", payload, "application/json; charset=utf-8")
    ...
    def _report(self, data: dict) -> None:
        ...
        name = f"lythosbearing_report.{fmt}"
    ...
    def _export_study(self, data: dict) -> None:
        ...
        name = f"lythosbearing_study.{kind}"
        with tempfile.TemporaryDirectory() as tmp:
...
def serve(host: str = "127.0.0.1", port: int = 8781, open_browser: bool = True,
          lang: str = "en") -> None:
