"""
Parametric and probabilistic (reliability) studies for Lythos Bearing.

A study is defined by a list of variables, each addressing one input of the
project configuration by a dotted path (``foundation.B``, ``loading.V``,
``soil_profile.1.phi`` …), and either
...
Every sample is a complete bearing capacity analysis — a few milliseconds —
...
from .engine import BearingAnalysis
...
OUTPUTS = [
    ("q_ult", "kPa"), ("q_net_ult", "kPa"), ("q_all", "kPa"), ("q_applied", "kPa"),
    ("FS", ""), ("utilisation", ""), ("FS_sliding", ""), ("ecc", ""),
]

#: The limit states the reliability analysis counts:
#: key -> (output, capacity from the configuration, failure when above)
LIMIT_STATES = {
    "ls_bearing": ("utilisation", lambda cfg: 1.0, True),
    "ls_sliding": ("FS_sliding", lambda cfg: float(cfg.get("criteria", {})
                                                   .get("FS_sliding", 1.5)), False),
    "ls_eccentricity": ("ecc", lambda cfg: 1.0 / float(cfg.get("criteria", {})
                                                       .get("ecc_limit", 6.0)), True),
}

#: The inputs of each section a study may vary
FOUNDATION_KEYS = ["B", "L", "Df", "base_tilt", "ground_slope"]
LOADING_KEYS = ["V", "Hb", "Hl", "Mb", "Ml"]
LAYER_KEYS = ["thickness", "gamma", "gamma_sat", "c", "phi", "cu", "E", "nu"]
SEISMIC_KEYS = ["kh", "kv"]
...
def available_variables(cfg: Dict[str, Any]) -> List[Tuple[str, str]]:
    ...
    out = [(f"foundation.{key}", "foundation") for key in FOUNDATION_KEYS]
    out += [(f"loading.{key}", "loading") for key in LOADING_KEYS]
    ...
    if (cfg.get("seismic") or {}).get("enabled"):
        out += [(f"seismic.{key}", "seismic") for key in SEISMIC_KEYS]
    return out
...
def pretty_label(cfg: Dict[str, Any], path: str, L: Dict[str, str]) -> str:
    ...
    group = {"foundation": "grp_foundation", "loading": "grp_loading",
             "groundwater": "grp_water", "seismic": "grp_seismic"}.get(parts[0])
...
def evaluate(base_cfg: Dict[str, Any], values: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        analysis = BearingAnalysis(cfg)
        res = analysis.run(with_width=False)
    ...
    checks = res["checks"]
    row.update({
        "q_ult": res["q_ult"], "q_net_ult": res["q_net_ult"],
        "q_all": checks["bearing"]["q_all"], "q_applied": res["applied"]["q"],
        "FS": checks["bearing"]["actual"],
        "utilisation": checks["bearing"]["utilisation"],
        "FS_sliding": checks["sliding"]["actual"],
        "ecc": checks["eccentricity"]["actual"],
        "error": "",
    })
    return row
