#!/usr/bin/env python3
"""Run Lythos Bearing without installing it.

    python main.py                         opens the interface in a browser
    python main.py run project.bearing     analyses a project file
    python main.py study project.bearing   runs the study a project file defines
...
def explain(missing: list) -> None:
    print("Lythos Bearing needs these Python packages, which are not installed:\n",
          file=sys.stderr)
    for name in missing:
...
def main() -> int:
    if sys.version_info < (3, 10):
        print(f"Lythos Bearing needs Python 3.10 or later; this is "
    ...
    if gaps:
        explain(gaps)
        return 1
    from lythosbearing.cli import main as run_cli
