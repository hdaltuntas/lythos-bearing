...
The distribution is named `lythosbearing`; so are the package you import and the
directory in the repository. The command you type is `lythos-bearing`.
...
with one scoped to `lythosbearing`. Put it in `~/.pypirc`:
...
1. Raise `version` in `pyproject.toml` and `__version__` in
   `lythosbearing/__init__.py`; the test suite checks that the two agree.
...
5. Check what was actually published, in an empty environment:
   ...
   /tmp/check/bin/pip install lythosbearing
   /tmp/check/bin/lythos-bearing example -o /tmp/check.bearing
   /tmp/check/bin/lythos-bearing run /tmp/check.bearing -o /tmp/check.pdf
   /tmp/check/bin/lythos-bearing web --no-browser        # then open the address
   ```
...
pip install --index-url https://test.pypi.org/simple/ \
            --extra-index-url https://pypi.org/simple/ lythosbearing
```
...
`lythosbearing/web/static/*` is declared as package data in `pyproject.toml`.
...
```bash
python -c "import zipfile; print(zipfile.ZipFile('dist/lythosbearing-0.1.0-py3-none-any.whl').namelist())"
```
